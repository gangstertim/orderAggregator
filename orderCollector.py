...
def save_order():
    ...
    if response:
        pass
    elif re.match("orderbot[,:]\s*status", post):
        ...
        return post_message('Order with this format: `orderBot: restaurant: order`. For example: `orderBot: Mizu: Lunch Special, Spicy Tuna Roll, Yellowtail Roll, Salmon Roll, special instructions "Label Jim, extra spicy"`.  To see if/what you have ordered, simply type `orderBot: status`')
    elif order:
